...
export default async function handler(req, res) {
  ...
  try {
    ...
    // FIXED: Target basis par Bucket aur Public URL select karne ka logic
    let bucket = process.env.R2_BUCKET;
    let configuredPublicUrl = process.env.R2_PUBLIC_URL;

    if (isAvatar) {
      bucket = process.env.R2_AVATAR_BUCKET_NAME || 'avatar';
      configuredPublicUrl = process.env.NEXT_PUBLIC_R2_AVATAR_PUBLIC_URL || process.env.R2_PUBLIC_URL;
    } else if (isStory) {
      bucket = process.env.R2_STORY_BUCKET_NAME || process.env.R2_BUCKET;
    } else if (isChat) {
      bucket = process.env.R2_CHAT_BUCKET_NAME || process.env.R2_BUCKET;
    }
    ...
    const missing = [
      ...
      ['Bucket Name', bucket],
      ['Public URL', configuredPublicUrl]
    ].filter(([, value]) => !value).map(([name]) => name);

    if (missing.length) {
      return res.status(500).json({ error: `Missing R2 environment variable(s): ${missing.join(', ')}` });
    }
    ...
    // Pure R2 public domain par upload path format:

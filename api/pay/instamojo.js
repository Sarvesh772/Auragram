...
async function resolveUser(req, body) {
  ...
  if (body.userId && body.userEmail) {
    return { user: { id: body.userId, email: body.userEmail } };
  }
...
export default async function handler(req, res) {
  ...
  const body = req.body || {};
  const { amount, planType, userName } = body;
  ...
  try {
    const redirectUrl = `https://auragram.in/api/pay/verify-instamojo?user_id=${encodeURIComponent(user.id)}&plan=${plan}`;
